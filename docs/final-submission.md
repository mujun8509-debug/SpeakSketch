...
当前 `/api/asr` 仍为 Mock 返回，不连接真实讯飞 WebSocket。`/api/style-image` 已支持可选 Seedream 5.0 图生图 provider：配置有效 `SEEDREAM_API_KEY` 时可由后端把当前画布图片作为输入传给 Seedream；AI 风格化请求会附带 `semanticPrompt`、`sceneDescription` 和 `actionSummary`，帮助模型理解海边、公园、猫等抽象草图元素。未配置时返回 Mock 结果，生成失败时返回受控错误。默认未配置真实 provider 时，前端使用浏览器识别、调试输入或 Mock 模式保证基础流程可用。
...
- AI 风格化语义增强只影响发送给后端的临时导出图和 prompt，不覆盖原始 Fabric 画布
