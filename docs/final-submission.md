...
当前 `/api/asr` 和 `/api/style-image` 均为 Mock 返回，不连接真实讯飞 WebSocket，不调用真实 OpenAI 图像生成 API。默认未配置真实 provider 时，前端使用浏览器识别、调试输入或 Mock 模式保证基础流程可用。
...
- 真实 OpenAI 图像生成当前为接口预留 / mock 骨架
- 图像生成依赖后端真实 provider 接入
