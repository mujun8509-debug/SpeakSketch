...
当前 `/api/asr` 仍为 Mock 返回，不连接真实讯飞 WebSocket。`/api/style-image` 已支持可选 OpenAI 图像 provider：配置有效 `OPENAI_API_KEY` 时可由后端调用 OpenAI 图像 API；未配置时返回 Mock 结果，生成失败时返回受控错误。默认未配置真实 provider 时，前端使用浏览器识别、调试输入或 Mock 模式保证基础流程可用。

## 安全说明

- 前端不保存任何 API Key
- 前端不保存或传递 `OPENAI_API_KEY`
...
- 真实讯飞 ASR 当前为接口预留 / mock 骨架
- OpenAI 图像生成是可选后端增强，不影响结构化绘图主流程
- 真实图像生成需要在 `server/.env` 配置有效 `OPENAI_API_KEY`，并可能产生费用
...
| OpenAI 真实图像生成 | 需配置有效 `OPENAI_API_KEY` 后测试，当前不声称已完整验证 |
