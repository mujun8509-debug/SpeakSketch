# SpeakSketch 测试清单

## 最终提交测试清单

### 启动与构建
- [ ] 前端 `npm run build`
- [ ] 前端 `npm run dev`
- [ ] 后端 `npm install`
- [ ] 后端 `npm run dev`

### 后端接口
- [ ] `GET /api/health`
- [ ] `POST /api/asr` mock
- [ ] `POST /api/style-image` mock
- [ ] `/api/style-image` 生成失败或非法输入时返回受控错误

### OpenAI 图像 provider
- [ ] 未配置 `OPENAI_API_KEY` 时 `/api/style-image` 返回 Mock 结果
- [ ] 非法 data URL 或 provider 失败时返回受控错误
- [ ] 前端不保存 `OPENAI_API_KEY`
- [ ] 配置有效 `OPENAI_API_KEY` 后真实生成可用（仅在使用者自行配置 Key 后测试；未测试时不要勾选）

### 核心功能
- [ ] 中文指令：`画一个红色圆形`
- [ ] 英文指令：`draw a blue rectangle`
- [ ] 空间关系：`画一个男人站在树旁边`
- [ ] Demo 模式
- [ ] 撤销 / 重做
- [ ] 重放
- [ ] 导出
- [ ] Mock 图像生成
- [ ] 未配置 API 不崩溃

## 环境测试

### 基础环境
- [ ] `npm install` - 安装依赖成功
- [ ] `npm run dev` - 开发服务器启动成功
- [ ] `npm run build` - 生产构建成功

### 页面加载
- [ ] 页面刷新后能正常打开
- [ ] 布局正常，无明显样式问题
- [ ] 所有组件正常渲染

## 功能测试

### 基础绘图
- [ ] 画一个红色圆形
- [ ] 画一个蓝色矩形
- [ ] 画一个三角形
- [ ] 画一条直线
- [ ] 写上"人工智能绘图"

### 复杂对象
- [ ] 画一个男人
- [ ] 画一只猫
- [ ] 画一只狗
- [ ] 画一辆红色汽车
- [ ] 画一朵花

### 复杂场景
- [ ] 画一个公园，有人、树、花、草地和小鸟
- [ ] 画一个海边，有太阳、云、船、海平线和飞鸟
- [ ] 画一个校园，有教学楼、树、草地和学生

### 空间关系
- [ ] 画一个男人站在树旁边
- [ ] 画一只猫在汽车旁边
- [ ] 画一艘船在河上
- [ ] 画几只鸟在天空中

### 编辑操作
- [ ] 把刚才那个图形改成蓝色
- [ ] 把最大的图形放大一点
- [ ] 删除最左边的图形
- [ ] 把刚才那个图形往右移动一点

### 系统指令
- [ ] 撤销
- [ ] 重做
- [ ] 清空画布
- [ ] 导出图片
- [ ] 重放全部

### 一键演示
- [ ] 点击一键演示按钮
- [ ] 演示过程正常执行
- [ ] 语音提示"演示完成"

## 边界测试

### 空状态
- [ ] 空画布撤销不报错
- [ ] 空画布重做不报错
- [ ] 清空画布后能继续绘制

### 异常情况
- [ ] Chrome 语音识别权限异常时调试输入可用
- [ ] 输入无法识别的指令不导致崩溃
- [ ] 解析失败时有友好提示

### 性能测试
- [ ] 绘制多个对象后页面响应正常
- [ ] 重放大量指令时不卡顿
- [ ] 导出大型画布不报错

## 兼容性测试

### 浏览器兼容性
- [ ] Chrome（推荐）
- [ ] Edge
- [ ] Firefox（语音识别可能受限）
- [ ] Safari（语音识别可能受限）

### 设备兼容性
- [ ] 桌面端
- [ ] 平板端（触控操作）
- [ ] 移动端（语音识别可能受限）

## 历史管理测试

### 撤销重做
- [ ] 连续撤销多个操作
- [ ] 撤销后重做恢复
- [ ] 清空后撤销无效

### 重放功能
- [ ] 重放全部正常执行
- [ ] 重放后画布状态正确
- [ ] 重放过程不重复追加历史

## 数据持久化测试

### 导出功能
- [ ] 导出 PNG 图片正常
- [ ] 导出图片内容完整
- [ ] 导出图片格式正确

### 状态管理
- [ ] 指令历史记录完整
- [ ] 日志记录正常
- [ ] 状态更新及时

## 安全测试

### 输入安全
- [ ] 恶意输入不导致 XSS
- [ ] 大文本输入不导致崩溃
- [ ] 特殊字符输入正常处理

### 权限安全
- [ ] 无需登录即可使用
- [ ] 未配置后端接口时，本地绘图与调试输入不依赖服务器
- [ ] 配置云端 ASR 或图像生成接口时，仅通过后端代理调用
- [ ] 前端不保存讯飞或 OpenAI API Key
- [ ] 不存在 `VITE_OPENAI_API_KEY` 或前端直连 OpenAI 的配置

## 测试通过标准

### 必须通过
- [ ] `npm run build` 构建成功
- [ ] 页面正常加载
- [ ] 一键演示正常运行
- [ ] 基础绘图功能正常
- [ ] 撤销/重做功能正常

### 建议通过
- [ ] 核心示例指令正常执行
- [ ] 复杂场景绘制正常
- [ ] 空间关系解析正常
- [ ] 导出功能正常
- [ ] 重放功能正常

## 问题记录

| 问题描述 | 严重程度 | 状态 | 备注 |
|----------|----------|------|------|
| | | | |
| | | | |
| | | | |

## 测试环境

| 项目 | 版本 |
|------|------|
| Node.js | 20.x |
| npm | 10.x |
| Chrome | 120+ |
| 操作系统 | Windows/macOS/Linux |

## 测试人员

- 测试日期：
- 测试人员：
- 测试环境：
