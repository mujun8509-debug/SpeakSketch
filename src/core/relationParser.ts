...
export function parseRelation(text: string): RelationResult | null {
  ...
  // 模式1: "画一个A站在B旁边" 或 "画一个A在B旁边"
  const patterns = [
    /(?:画(?:一(?:个|只|辆|朵|座|条|艘|片)|几只))(.{1,10})(?:站在?|在)(.{1,10})(?:旁边|左边|右边|上方|下方|上面|下面)/g,
    /(?:画(?:一(?:个|只|辆|朵|座|条|艘|片)|几只))(.{1,10})(?:停|飞)在(.{1,10})(?:旁边|天空|地面|河上|水上)/g,
    /(?:画(?:一(?:个|只|辆|朵|座|条|艘|片)|几只))(.{1,10})(?:在|飞到)(.{1,10})(?:旁边|天空|地面|河上|水上)/g,
  ];
